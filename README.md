...
## 1.5）分析
```markdown
1.优点：prompt_v1在大部分场景下生成的token数量比promt_v0要多，同时也更加细致。会关注到次要元素的结构信息，以及对一些抽象风格进行描述，在人工图像数据集上普遍质量优于prompt_0
2.问题：比如有时候过于发散，虽然约束了华丽词语，但是依旧存在生成的caption修饰词过多的问题。在一些自然场景下例如nature中的worker，prompt_v1自己加了很多“体现了团结的重要性”的无效词语。
3.接下来的改进方案：
1）改进写法
